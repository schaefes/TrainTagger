...
def derive_rate(minbias_path, cuts, n_entries=100000, tree='outnano/Jets'):
    ...
    event_id, grouped_arrays  = group_id_values(raw_event_id, array_fields, num_elements=n_objects)
    jet_btag, jet_pt, jet_eta = grouped_arrays
    jet_pt = ak.sort(jet_pt, axis=1, ascending=False)
    n_events = len(np.unique(event_id))
    ht = ak.sum(jet_pt, axis=1)
    btag_sum = ak.sum(jet_btag, axis=1)

    # Apply the cuts
    ht_mask = (ht > ht_cut)
    btag_mask = (btag_sum > btag_cut)
    pt1_mask, pt2_mask, pt3_mask, pt4_mask = (jet_pt[:,0] > pt1_cut), (jet_pt[:,1] > pt2_cut), (jet_pt[:,2] > pt3_cut), (jet_pt[:,3] > pt4_cut)

    mask = ht_mask & btag_mask & pt1_mask & pt2_mask & pt3_mask & pt4_mask

    # convert to rate [kHz]
    rate = (np.sum(mask) / n_events) * MINBIAS_RATE
...
if __name__ == "__main__":
    ...
    if args.deriveWPs:
        rate = derive_rate(args.minbias, 2, {'ht': 200, 'btag': 0.5, 'pt1': 20, 'pt2': 20}, n_entries=args.n_entries)
        derive_cc_WPs(args.model_dir, args.minbias, target_rate=rate, n_entries=args.n_entries)
    elif args.eff:
